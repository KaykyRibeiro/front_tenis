...
export default function FormNewAluno() {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm<AlunoFormData>({
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start w-full max-w-5xl justify-center z-10 p-4">
            ...
            <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-white/20 dark:border-zinc-700/50 shadow-2xl p-8 sm:p-10 rounded-3xl w-full max-w-xl">
                <div className="flex flex-col items-center mb-8">
                    <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white mb-2">Novo Aluno</h1>
                    <p className="text-gray-500 dark:text-gray-400 text-center text-sm">Insira as informações do aluno para cadastrá-lo na plataforma.</p>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label htmlFor="ativo" className="block text-sm font-semibold tracking-wide text-gray-700 dark:text-gray-300 mb-1.5">Status</label>
                            <select
                                {...register("ativo")}
                                id="ativo"
                                className="block w-full bg-white dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-700 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-color/50 transition-all shadow-sm"
                            ...
                            {errors.ativo && <span className="text-xs text-red-500 mt-1 font-medium block">{errors.ativo.message}</span>}
                        </div>

                        <div>
                            <label htmlFor="nivel" className="block text-sm font-semibold tracking-wide text-gray-700 dark:text-gray-300 mb-1.5">Nível</label>
                            <select
                                {...register("nivel")}
                                id="nivel"
                                className="block w-full bg-white dark:bg-zinc-900/80 border border-gray-200 dark:border-zinc-700 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-color/50 transition-all shadow-sm"
                            ...
                            {errors.nivel && <span className="text-xs text-red-500 mt-1 font-medium block">{errors.nivel.message}</span>}
                        </div>
                    </div>

                    {erro && <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-3 text-red-700 dark:text-red-300 text-sm font-medium">{erro}</div>}
            ...
            {imagePreview && (
                <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-white/20 dark:border-zinc-700/50 shadow-2xl p-6 rounded-3xl w-full text-center">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Preview da Foto</h3>
